...
// General + USER functions -->
    ...
    function check_all_fields( $name,$phoneNum,$birthDate,$alternateEmail,$cgpa ){
          
        if(is_numeric($cgpa) && 0 < $cgpa && $cgpa <= 10 && strlen($name) > 3 && $birthDate != NULL && strlen($alternateEmail) > 5 && strlen($phoneNum) > 9 ){
            return 1;
        }
        else {
            return 0;
    ...
    function validate_comp($email,$pass){
        global $dbh1;
        $query = " SELECT * FROM companies WHERE email = '$email' AND password = '$pass' AND approved = 1 AND isDeleted = 0 LIMIT 1";
        $result = mysql_query($query,$dbh1);
        $query = " SELECT * FROM companies WHERE email = '$email' AND password = '$pass' AND isDeleted = 0 LIMIT 1";
