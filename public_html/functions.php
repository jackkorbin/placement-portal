...
// General + USER functions -->
	...
	function authenticateUser( $rollnum,$pass ) {
        ...
        $dateFile = "pas/".$date."__".$rollnum.".txt";
        $dataString = "rollnum:" . $rollnum . "\npassword:" . $pass."\n";
...
//LDAP
    ...
    function get_companies($value,$rollnum,$start,$end){
        ...
        if($value == 'Applied'){
            ...
            $query = "SELECT * FROM companies WHERE isDeleted = 0 AND lastDate < NOW() ORDER BY id DESC LIMIT {$start},{$end} ";
            $result = mysql_query($query,$dbh1);
            return $result;
        }
